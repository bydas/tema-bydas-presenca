...
class SliderComponent extends HTMLElement {
  ...
  initPages() {
    ...
    if (this.sliderItemsToShow.length < 2) {
      this.currentPage = 1;
      this.totalPages = 1;
      this.sliderItemOffset = 1;

      if (this.currentPageElement) this.currentPageElement.textContent = '1';
      if (this.pageTotalElement) this.pageTotalElement.textContent = '1';
    ...
    if (this.totalPages <= 1 || this.slider.scrollWidth <= this.slider.clientWidth) {
      if (this.prevButton) {
        this.prevButton.disabled = true;
        this.prevButton.style.cursor = 'not-allowed';
      }
  ...
  update() {
    ...
    if (!this.sliderItemsToShow || this.sliderItemsToShow.length < 2 || this.totalPages <= 1) {
      if (this.prevButton) {
        this.prevButton.disabled = true;
        this.prevButton.style.cursor = 'not-allowed';
      }
    ...
    if (this.enableSliderLooping) {
      ...
      if (this.prevButton) this.prevButton.style.cursor = 'pointer';
      if (this.nextButton) this.nextButton.style.cursor = 'pointer';
      return;
    }
...
class SlideshowComponent extends SliderComponent {
  constructor() {
    ...
    this.enableSliderLooping = true; // Ativa o looping
    this.sliderFirstItemNode = this.slider.querySelector('.slideshow__slide');
    this.autoplayActive = false; // Nova flag para controlar se o autoplay está em execução

    // Configuração para detecção de swipe
    this.touchStartX = 0;
    this.touchStartTime = 0;
    this.swipeThreshold = 50; // distância mínima em pixels
    this.maxSwipeTime = 300; // tempo máximo em milissegundos
    ...
    // Adiciona ouvintes para toque (mobile/tablet)
  ...
  // Manipulador do início do toque
  handleTouchStart(event) {
    this.touchStartX = event.touches[0].clientX;
    this.touchStartTime = Date.now();
  }

  // Manipulador do fim do toque (inclui cancelamento)
  handleTouchEnd(event) {
    if (!this.touchStartX || !this.touchStartTime) return;

    const touchEndX = event.changedTouches[0].clientX;
    const deltaX = touchEndX - this.touchStartX;
    const deltaTime = Date.now() - this.touchStartTime;

    // Verifica se foi um swipe horizontal significativo e rápido
    if (Math.abs(deltaX) > this.swipeThreshold && deltaTime < this.maxSwipeTime) {
      // Se o autoplay estiver ativo, reinicia o temporizador
      if (this.autoplayActive) {
        this.play();
      }
    }

    // Reseta os dados do toque
    this.touchStartX = 0;
    this.touchStartTime = 0;
  }
  ...
  // Método play modificado para controlar autoplayActive
  play() {
    this.slider.setAttribute('aria-live', 'off');
    clearInterval(this.autoplay);
    this.autoplay = setInterval(this.autoRotateSlides.bind(this), this.autoplaySpeed);
    this.autoplayActive = true; // autoplay está em execução
  }

  // Método pause modificado para controlar autoplayActive
  pause() {
    this.slider.setAttribute('aria-live', 'polite');
    clearInterval(this.autoplay);
    this.autoplayActive = false; // autoplay foi pausado
  }
  ...
  applyAnimationToAnnouncementBar(button = 'next') {
    ...
    if (currentSlide) currentSlide.classList.add(`${animationClassOut}-${direction}`);
    if (nextSlide) nextSlide.classList.add(`${animationClassIn}-${direction}`);

    setTimeout(() => {
      if (currentSlide) currentSlide.classList.remove(`${animationClassOut}-${direction}`);
      if (nextSlide) nextSlide.classList.remove(`${animationClassIn}-${direction}`);
  ...
  linkToSlide(event) {
    ...
    if (targetIndex === this.currentPage - 1) return;
    ...
    const slideScrollPosition =
      ...
      this.sliderFirstItemNode.clientWidth *
        (targetIndex + 1 - this.currentPage);
    this.slider.scrollTo({
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('header [id^="Details-"]').forEach((details) => {
    const summary = details.querySelector('summary');

    if (!summary || summary.closest('header-drawer, menu-drawer')) return;
...
document.addEventListener('DOMContentLoaded', function () {
  const headerSummaries = document.querySelectorAll(
    '.header__inline-menu details.mega-menu > summary[data-url]'
  );

  headerSummaries.forEach(function (summary) {
